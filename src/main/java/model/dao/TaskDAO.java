...
public class TaskDAO {
	...
	public int create(Task task, String groupId) throws SQLException, ParseException {
		...
		Object[] param = new Object[] { task.getName(), task.getStartDate(), task.getEndDate(), task.getDescription(),
				groupId };
	...
	public List<Task> getList(String groupId) {
		...
		try {
			...
			while (rs.next()) {
				Task task = new Task(rs.getString("taskId"), rs.getString("taskName"), rs.getString("startDate"),
						rs.getString("endDate"), rs.getString("taskDescription"));
				taskList.add(task);
			}
			return taskList;
	...
	public Task getTask(String taskId) {
		...
		try {
			...
			if (rs.next()) {
				Task task = new Task(taskId, rs.getString("taskName"), rs.getString("startDate"),
						rs.getString("endDate"), rs.getString("taskDescription"));
				return task;
			}
		} catch (Exception ex) {
	...
	public int submitTask(String taskId, Submit submit) throws SQLException {
		...
		jdbcUtil.setSqlAndParameters(sql,
				new Object[] { submit.getUserId(), submit.getFilePath(), submit.getSubmitContents(), taskId });
		try {
			int result = jdbcUtil.executeUpdate();
			return result;
		} catch (Exception ex) {
...
//과제 수정
	public int update(String submitContents, String submitId) throws SQLException {
		String sql = "UPDATE Submit " + "SET submitContents=? "
				+ "WHERE submitId=?";
		Object[] param = new Object[] { submitContents, submitId};
	...
	public Submit getSubmit(String taskId, String userId) {
		...
		try {
			...
			if (rs.next()) {
				Submit submit = new Submit(rs.getString("userId"), rs.getString("filePath"),
						rs.getString("submitContents"), rs.getString("submitDate"), rs.getString("taskId"),
	...
	public List<Submit> getSubmitList(String taskId) {
		...
		try {
			...
			while (rs.next()) {
				Submit submit = new Submit(rs.getString("userId"), rs.getString("filePath"),
						rs.getString("submitContents"), rs.getString("submitDate"), taskId, rs.getString("submitId"));
